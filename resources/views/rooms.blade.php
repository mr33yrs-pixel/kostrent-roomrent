<x-layouts.app :title="'JaiPremiumKost - ' . __('messages.rooms.title')">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-16">
                        ...
                        <div class="bg-white rounded-3xl shadow-sm hover:shadow-md transition-shadow duration-300 overflow-hidden group">
                            <div class="relative h-64 bg-gray-200 {{ !$room->is_available ? 'grayscale' : '' }}">
                                @if(!empty($room->images) && isset($room->images[0]))
                                    <img src="{{ \Illuminate\Support\Facades\Storage::url($room->images[0]) }}" alt="{{ $room->name }}" loading="lazy" decoding="async" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                                @else
                ...
                @if($premiumRooms->hasPages())
                    <div class="mt-8">
                        {{ $premiumRooms->appends(['standard_page' => request('standard_page')])->links() }}
                    </div>
                @endif
            </div>
            ...
            <div>
                ...
                @if($standardRooms->hasPages())
                    <div class="mt-8">
                        {{ $standardRooms->appends(['premium_page' => request('premium_page')])->links() }}
                    </div>
                @endif
            </div>
        </div>
